...
import org.sonatype.nexus.plugins.${pluginFormat}.internal.AssetKind;
import org.sonatype.nexus.plugins.${pluginFormat}.internal.util.${pluginClass}DataAccess;
import org.sonatype.nexus.plugins.${pluginFormat}.internal.util.${pluginClass}PathUtils;
...
/**
 * ${pluginClass} {@link ProxyFacet} implementation.
 *
 * @since 0.0.1
 */
@Named
public class ${pluginClass}ProxyFacetImpl
    extends ProxyFacetSupport
    implements ${pluginClass}ProxyFacet
{
  private ${pluginClass}PathUtils ${pluginFormat}PathUtils;

  private ${pluginClass}DataAccess ${pluginFormat}DataAccess;

  @Inject
  public ${pluginClass}ProxyFacetImpl(final ${pluginClass}PathUtils ${pluginFormat}PathUtils,
                             final ${pluginClass}DataAccess ${pluginFormat}DataAccess)
  {
    this.${pluginFormat}PathUtils = checkNotNull(${pluginFormat}PathUtils);
    this.${pluginFormat}DataAccess = checkNotNull(${pluginFormat}DataAccess);
  }
  ...
  public Content getAsset(final String assetPath) {
    ...
    Asset asset = ${pluginFormat}DataAccess.findAsset(tx, tx.findBucket(getRepository()), assetPath);
    if (asset == null) {
      return null;
    }
    return ${pluginFormat}DataAccess.toContent(asset, tx.requireBlob(asset.requireBlobRef()));
  }
                                      ...
                                      final Component component) throws IOException
  {
    ...
    return ${pluginFormat}DataAccess.saveAsset(tx, asset, tempBlob, content);
  }
  ...
  public void setCacheInfo(final Content content, final CacheInfo cacheInfo) throws IOException {
    ...
    if (asset == null) {
      log.debug(
          "Attempting to set cache info for non-existent ${pluginClass} asset {}", content.getAttributes().require(Asset.class)
      );
      return;
    }
